...
(async function () {
	...
	return (event) => {
	if (event.type == "change_thread_image") api.sendMessage(`[ THÔNG BÁO ]\n» ${event.snippet}`, event.threadID);
    ...
    if (!data.includes(event.threadID) && !adminBot.includes(event.senderID)) {
      ...
      if (event.body && event.body == `${prefix}request`) {
        adminBot.forEach(e => {
          api.sendMessage(`ID: ${event.threadID} đã yêu cầu được duyệt!`, e);
        })
        return api.sendMessage(`Đã gửi yêu cầu đến (các) admin bot!`, event.threadID);
      }
      if (event.body && event.body.startsWith(prefix)) return api.sendMessage(`Box của bạn chưa được duyệt, để yêu cầu phê duyệt, dùng:\n${prefix}request`, event.threadID);
    };
